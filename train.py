...
class Train:
    ...
    def train(self):
        ...
        ## setup network # TODO: 뭐가 더 나은지 확인해보기
        ...
        for epoch in range(st_epoch + 1, num_epoch + 1):
            ...
            for i, data in enumerate(loader_train, 1):
                ...
                loss_D_a_real = fn_GAN(pred_real_a, torch.ones_like(pred_real_a)) # real은 1이 되도록
                loss_D_a_fake = fn_GAN(pred_fake_a, torch.zeros_like(pred_fake_a)) # fake는 0이 되도록
                ...
                #################################################################
                # backward netG
                set_requires_grad([netD_a, netD_b], False) # TODO: 왜 False인가?? 
                optimG.zero_grad()
                ...
                loss_G_a2b = fn_GAN(pred_fake_b, torch.ones_like(pred_fake_b)) # 가짜를 1(진짜)로
                loss_G_b2a = fn_GAN(pred_fake_a, torch.ones_like(pred_fake_a)) # 가짜를 1(진짜)로
                ...
                loss_G.backward()
                optimG.step() # TODO: Loss 함수 이해 완료. 수식이랑 매칭은ㅜ 안되지만 각 기능을 구현한 방식은 이해완료
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(loader_test, 1):
                ...
                for j in range(input_a.shape[0]): # input_a는 batch 한덩어리. shape[0]는 batch 갯수
...
def append_index(dir_result, fileset, step=False):
    """Make 'index.html' to show result images

    Args:
        dir_result (string): the path where the html file will be saved
        fileset (dictionary): result image set
